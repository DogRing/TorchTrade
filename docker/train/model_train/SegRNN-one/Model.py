...
class Configs:
    def __init__(self,config):
        ...
        self.enc_in = config.get('enc_in')
        self.d_model = config.get('d_model')
        self.dropout = config.get('dropout')
        self.rnn_type = config.get('rnn_type')
        self.dec_way = config.get('dec_way')
        self.seg_len = config.get('seg_len')
        self.channel_id = config.get('channel_id')
        self.revin = config.get('revin')
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.attention = nn.MultiheadAttention(embed_dim=self.enc_in, num_heads=1,batch_first=True)
        self.final_linear = nn.Linear(self.pred_len,1)
